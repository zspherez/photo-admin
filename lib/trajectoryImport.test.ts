...
const TEST_IMPORT_NOW = () => new Date("2026-07-21T12:00:00.000Z");

function importTestTrajectoryManifest(
  bytes: Buffer,
  options: NonNullable<Parameters<typeof importTrajectoryManifest>[1]> = {},
) {
  return importTrajectoryManifest(bytes, {
    now: TEST_IMPORT_NOW,
    ...options,
  });
}
...
test("names never provide fallback identity mapping", async () => {
  ...
  await assert.rejects(
    importTestTrajectoryManifest(raw(value), { persistence }),
    (error: unknown) =>
...
test("non-suggested mapping issues persist at the 2% default threshold", async () => {
  ...
  const summary = await importTestTrajectoryManifest(raw(value), {
    persistence,
  });
...
test("lease busy and lease loss never write or displace a ready run", async () => {
  ...
  busy.busy = true;
  const busySummary = await importTestTrajectoryManifest(raw(value), {
    persistence: busy,
  });
  ...
  await assert.rejects(
    importTestTrajectoryManifest(raw(value), { persistence: lost }),
    IntegrationSyncLeaseLostError,
  );
...
test("same producer run and digest is a no-op while a different digest conflicts", async () => {
  ...
  const persistence = new MemoryPersistence(exactSnapshot(value));
  const first = await importTestTrajectoryManifest(bytes, { persistence });
  const second = await importTestTrajectoryManifest(bytes, { persistence });
  ...
  await assert.rejects(
    importTestTrajectoryManifest(raw(changed), { persistence }),
    (error: unknown) =>
      error instanceof TrajectoryImportError &&
