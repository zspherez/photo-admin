...
function organizationDomainLooksRelated(domain, organizationName) {
  const domainLabels = domain
    ...
    .filter(
      (label) =>
        label.length >= 3 && !GENERIC_DOMAIN_LABELS.has(label),
    );
  const organizationTokens = normalizedIdentityTokens(organizationName)
    .filter((token) => !GENERIC_ORGANIZATION_TOKENS.has(token))
    .map((token) => token.replace(/[^a-z0-9]/g, ""))
    .filter(Boolean);
  const combined = organizationTokens.join("");
  return domainLabels.some(
    (domainLabel) =>
      combined.includes(domainLabel) ||
      domainLabel.includes(combined) ||
      organizationTokens.some(
        (token) => token.length >= 3 && domainLabel.includes(token),
      ),
  );
}
  ...
  for (const [index, candidate] of submission.candidates.entries()) {
    ...
    const documentedAlternate =
      ...
      sources.some((source) => {
        const sourceHost = new URL(source.url).hostname;
        return (
          domainsAssociated(websiteHost, sourceHost) &&
          source.observedDomains.some((domain) =>
            domainsAssociated(emailDomain, domain),
          ) &&
          sourceHasAllTokens(source, context.organizationName)
        );
      });
    const associated = websiteHost
      ? domainsAssociated(emailDomain, websiteHost) || documentedAlternate
      : organizationDomainLooksRelated(
          emailDomain,
          context.organizationName,
        ) &&
        new Set(sameDomainOrganizationSources.map((source) => source.url))
          .size >= 2;
    if (!associated) {
      throw new Error(
