...
const GENERIC_ALIAS_REGION_TOKENS = new Set([
  "apac",
  "au",
  "ca",
  "de",
...
const GENERIC_ALIAS_COMPOUND_PARTS = [
  ["accounts", "payable"],
  ["accounts", "receivable"],
...
const GENERIC_ALIAS_COMPOUNDS = new Set(
  GENERIC_ALIAS_COMPOUND_PARTS.map((tokens) =>
    [...tokens].sort().join("|"),
  ),
);
const GENERIC_ALIAS_COMPACT_COMPOUNDS = new Set(
  GENERIC_ALIAS_COMPOUND_PARTS.flatMap((tokens) => [
    tokens.join(""),
    [...tokens].reverse().join(""),
  ]),
);
...
export function isGenericProfessionalAlias(localPart) {
  ...
  const departmentTokens = tokens.filter(
    (token) =>
      !GENERIC_ALIAS_REGION_TOKENS.has(token) && !/^\d+$/.test(token),
  );
  const compound = [...departmentTokens].sort().join("|");
  const compactRegionGeneric = [...GENERIC_ALIAS_REGION_TOKENS].some(
    (region) => {
      if (!compact.startsWith(region) || compact.length === region.length) {
        return false;
      }
      const remainder = compact.slice(region.length);
      return (
        GENERIC_ALIAS_ROOTS.has(remainder) ||
        GENERIC_ALIAS_COMPACT_COMPOUNDS.has(remainder)
      );
    },
  );
  if (
    ...
    (tokens.some((token) => GENERIC_ALIAS_ROOTS.has(token)) &&
      tokens
        .filter((token) => !GENERIC_ALIAS_ROOTS.has(token))
        .every(
          (token) =>
            GENERIC_ALIAS_REGION_TOKENS.has(token) || /^\d+$/.test(token),
        )) ||
    GENERIC_ALIAS_COMPOUNDS.has(compound) ||
    GENERIC_ALIAS_COMPACT_COMPOUNDS.has(compact) ||
    compactRegionGeneric
  ) {
    return true;
  }
...
export function canonicalEvidenceIdentityUrl(
  ...
  const url = new URL(canonicalPublicHttpsUrl(value, field));
...
export function normalizedEntityTokens(value) {
  return (
    String(value)
      .normalize("NFKD")
      .replace(/\p{M}/gu, "")
...
function parsedProvenance(provenance, expectedToken) {
  ...
  const sourceIdentityUrls = new Set();
  const fetchedSources = provenance.fetchedSources.map((source, index) => {
    ...
    const identityUrl = canonicalEvidenceIdentityUrl(url);
    if (
    ...
    const primaryEntityTokens = (source.primaryEntityTokens ?? []).map(
      (token) => {
        if (
          ...
          token.length < 1 ||
          token.length > 100 ||
          normalizedEntityTokens(token).length !== 1
        ) {
          throw new Error(
    ...
    const ownershipStatements = source.ownershipStatements.map(
      (statement, statementIndex) => {
        if (
          ...
          !Array.isArray(statement.entityTokens) ||
          statement.entityTokens.length > 50 ||
        ...
        return {
          ...
          entityTokens: statement.entityTokens.map((token) => {
            if (
              typeof token !== "string" ||
              token.length < 1 ||
              token.length > 100 ||
              ...
              throw new Error(
                `broker provenance fetchedSources[${index}].ownershipStatements[${statementIndex}].entityTokens is invalid`,
              );
            }
            return token;
          }),
    ...
    if (
      ...
      sourceByUrl.has(url) ||
      sourceIdentityUrls.has(identityUrl)
    ) {
    ...
    sourceIdentityUrls.add(identityUrl);
    return normalized;
  });
  if (
...
function sourceExplicitlyOwnsDomain(source, domain, organizationName) {
  return source.ownershipStatements.some(
    (statement) =>
      ...
      entityTokensEquivalent(
        statement.entityTokens,
        normalizedEntityTokens(organizationName),
      ),
  );
}
...
function sourcePrimaryEntityMatches(source, organizationName) {
  const required = normalizedEntityTokens(organizationName).filter(
    (token) => !GENERIC_ENTITY_TOKENS.has(token),
  );
  const actual = source.primaryEntityTokens.filter(
    (token) => !GENERIC_ENTITY_TOKENS.has(token),
  );
  return entityTokensEquivalent(actual, required);
}

function entityTokensEquivalent(left, right) {
  ...
  return (
    ...
    normalizedLeft.every((token) => normalizedRight.includes(token))
  );
}
