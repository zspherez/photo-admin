...
test("arbitrary email migrations are ordered, transactional, and constrained", () => {
  ...
  const trajectoryMigrationName =
    "20260721110000_artist_trajectory_runs";
  assert.ok(
    migrationNames.indexOf(queueMigrationName) <
  ...
  const structuredDirectOutreachMigrationName =
    "20260721180000_structured_direct_outreach_review";
  assert.ok(
    migrationNames.indexOf(directOutreachMigrationName) <
  ...
  const trajectoryIngestMigrationName =
    "20260721210000_trajectory_ingest_requests";
  assert.ok(
    migrationNames.indexOf(trajectoryFeedbackMigrationName) <
      migrationNames.indexOf(trajectoryIngestMigrationName),
  );
  const contactResearchCandidateStatusMigrationName =
    "20260722050000_contact_research_candidate_superseded_status";
  assert.ok(
    migrationNames.indexOf(trajectoryIngestMigrationName) <
      migrationNames.indexOf(contactResearchCandidateStatusMigrationName),
  );
  const contactAuditArtistDecisionMigrationName =
    "20260722210000_contact_audit_artist_decisions";
  assert.ok(
    migrationNames.indexOf(contactResearchCandidateStatusMigrationName) <
  ...
  assert.deepEqual(migrationNames.slice(-34), [
    ...
    "20260805160500_customize_all_recipients_no_manager_greeting",
    "20260816013000_outreach_all_to_delivery_mode",
    "20260817193000_festival_artist_rejections",
    "20260819190000_artist_merge_ui",
    "20260908190000_email_suppression_release",
    "20260918190000_email_attachments",
    "20260924125000_allow_audited_contact_deletion",
  ]);
