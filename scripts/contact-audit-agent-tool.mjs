...
const sessionId = process.env.CONTACT_AUDIT_AGENT_SESSION?.trim();

if (!socketPath || !sessionId) {
  throw new Error("contact audit broker is not configured");
}
...
const result = await new Promise((resolve, reject) => {
  const brokerRequest = request(
    {
      ...
      headers: {
        ...
        "x-contact-audit-session": sessionId,
      },
      timeout: 130_000,
    },
    (response) => {
  ...
  brokerRequest.on("timeout", () => {
    brokerRequest.destroy(new Error("contact audit broker timed out"));
  });
...
if (result.status < 200 || result.status >= 300) {
  throw new Error(
    `contact audit tool returned ${result.status}: ${result.text.slice(0, 1_000)}`
  );
}
