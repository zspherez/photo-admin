...
const sessionId =
  process.env.PROFESSIONAL_CONTACT_RESEARCH_AGENT_SESSION?.trim();
if (!socketPath || !sessionId) {
  throw new Error("professional contact research broker is not configured");
}
...
switch (action) {
  ...
  case "validate-result":
    try {
      input = { action: first, payload: JSON.parse(second ?? "") };
    } catch {
      throw new Error(
  ...
  default:
    throw new Error(
      "usage: claim [limit] | search <query> [limit] | fetch <url> | validate-result <submit-action> <json> | submit-candidates <json> | submit-exhausted <json>",
    );
}
...
const result = await new Promise((resolve, reject) => {
  const brokerRequest = request(
    {
      ...
      headers: {
        ...
        "x-professional-contact-session": sessionId,
      },
      timeout: 130_000,
    },
    (response) => {
  ...
  brokerRequest.on("timeout", () => {
    brokerRequest.destroy(new Error("professional contact broker timed out"));
  });
...
if (result.status < 200 || result.status >= 300) {
  throw new Error(
    `professional contact tool returned ${result.status}: ${result.text.slice(0, 1_000)}`,
  );
}
