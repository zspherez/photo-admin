...
export default async function NewFestivalPage({
  ...
  const requestedReturnTo = workflowReturnPath(
    (await searchParams).returnTo
  );
  const returnTo = requestedReturnTo.startsWith("/festivals")
    ? requestedReturnTo
    : "/festivals";
  return (
    <main className="mx-auto max-w-2xl px-6 py-10">
      <Link href={returnTo} className="text-xs text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100">← Festivals</Link>
      ...
      <FestivalForm returnTo={returnTo} />
    </main>
  );
}
